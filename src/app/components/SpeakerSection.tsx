...
export default function VoicesSlider() {
  // Set 1 (original set duplicated for smooth width)
  ...
  const renderCard = (item: VoiceItem, key: string) => (
    <div
      key={key}
      className="
        group 
  ...
  return (
    <section id="speakers" className="w-full py-16 overflow-hidden bg-white">
      ...
      <div className="marquee-container pt-24 pb-8">
        <div className="marquee-track flex items-center">
          {/* List Set 1 */}
          <div className="flex shrink-0 items-center gap-6 pr-6">
            {listSet.map((item, index) => renderCard(item, `set1-${item.id}-${index}`))}
          </div>

          {/* List Set 2 (Exact Duplicate for Seamless Loop) */}
          <div className="flex shrink-0 items-center gap-6 pr-6" aria-hidden="true">
            {listSet.map((item, index) => renderCard(item, `set2-${item.id}-${index}`))}
          </div>
        </div>
      </div>
    </section>
